...
def find_fixedline(calls,area_code):
    ...
    for fixedline in amount_of_fixedlines:
        ...
        if fixedline[0] == "7" and not (fixedline[0:4] in fixedlines):
            fixedlines.append(fixedline[0:4])
        if fixedline[0] == "8" and not (fixedline[0:4] in fixedlines):
            fixedlines.append(fixedline[0:4])
        if fixedline[0] == "9" and not (fixedline[0:4] in fixedlines):
            fixedlines.append(fixedline[0:4])
        if fixedline[0:2] == "140" and not (fixedline[0:3] in fixedlines):
...
def find_source_fixedline(calls,area_code):
    num_of_source_fixedline = 0
    for call in calls:
        if area_code in call[0]:
            num_of_source_fixedline += 1
    return num_of_source_fixedline

def perc_of_same_fixedline(calls,area_code):
    num_of_same_fixedlines = find_same_fixedline(calls,'(080)')
    num_of_source_fixedline = find_source_fixedline(calls,'(080)')
    perc = float(num_of_same_fixedlines/num_of_source_fixedline)
    perc = round(perc,2)
    return perc
